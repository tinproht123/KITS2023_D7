...
//    }
    @PostMapping("/activity")
    public ResponseEntity<?> createActivity(@RequestParam("MET")BigDecimal MET, @RequestParam("image") MultipartFile image, @RequestParam("name") String name, @RequestParam("type") String type) {
        String imageURl = imageService.create(image);
        return activityService.createActivity(MET, imageURl, name, type);
    }
